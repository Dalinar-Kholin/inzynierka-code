...
public class ChainServiceImpl : ChainService.ChainServiceBase
{
    ...
    public ChainServiceImpl(int serverId, int totalServers, string nextServer, int myPort, int ballotNumber, int numberOfCandidates)
    {
        _serverId = serverId;
        ...
        _myPort = myPort;
        _processor = new DataProcessingService(_serverId, _totalServers, _numberOfCandidates);
        ...
        if (_isLastServer) Task.Run(MonitorVoteCodesTimeout);
    }
    ...
    // da sie wyjac ale trzeba zmienic nazwe na uniwersalna
