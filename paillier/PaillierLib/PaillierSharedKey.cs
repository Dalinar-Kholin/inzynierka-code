...
public class PaillierSharedKey {
    ...
    public BigInteger n_square { get; }
    public BigInteger theta_inv { get; }
    public int degree { get; }
    public int fac_of_parties { get; }
    public BigInteger share { get; }

    public PaillierSharedKey(int serverNumber=0, string sharedKeyPath="../keys/paillier_keys_private.json") {
        ...
        this.n_square = BigInteger.Parse(serverKey["n_squared"]!.ToString());
    ...
    public PaillierSharedKey(BigInteger n, BigInteger n_square, int player_id, BigInteger theta_inv, int degree, BigInteger share, int fac_of_parties) {
        this.n = n;
    ...
    public BigInteger partial_decrypt(BigInteger ciphertext_value) {
        ...
        if (exp < 0) {
            ciphertext_value = mod_inv(ciphertext_value, this.n_square);
            exp = -exp;
        }
        BigInteger partial_decryption = BigInteger.ModPow(ciphertext_value, exp, this.n_square);
        return partial_decryption;
    }

    // zamienić na statyczną i może przenieć bo to tylko głosujący bd używał
    public BigInteger decrypt(Dictionary<int, BigInteger> partial_dict) {
        ...
        BigInteger combined_decryption = mult_list(partial_decryptions.Take(this.degree + 1).ToList()) % this.n_square;
    ...
    private BigInteger mult_list(List<BigInteger> list_, BigInteger? modulus = null) {
        BigInteger @out = 1;
        if (modulus == null) {
    ...
    private BigInteger mult_list(List<int> list_, BigInteger? modulus = null) {
        BigInteger @out = 1;
        if (modulus == null) {
            foreach (var element in list_) {
                @out = @out * element;
